...
public class DeploymentToggleBreakpointsTargetExtension implements IToggleBreakpointsTarget, IAdapterFactory {
	...
	private static Optional<INamedElement> getElementFromSelection(final Object selection) {
		if (selection instanceof final INamedElement element
				&& EcoreUtil.getRootContainer(element) instanceof AutomationSystem) {
			return Optional.of(element);
		}
		if (selection instanceof final EditPart editPart) {
			return getElementFromSelection(editPart.getModel());
		}
		return Optional.empty();
	}
}
